...
std::vector<std::unique_ptr<Server>> IdentityTestHelper::makeServers(std::size_t num) {
    assert(num <= 5);
    std::vector<std::unique_ptr<Server>> res;
    for (int i = 0; i < (int) num; ++i) {
        auto tmp = std::make_unique<Server>();
...
void IdentityTestHelper::runServers(std::vector<std::unique_ptr<Server>> &srvs) {
    for (auto &&srv : srvs)
        srv->run();
}
...
void IdentityTestHelper::sendHeartBeat(const std::vector<ServerId> &srvs,
    ...
    for (auto & srv : srvs) {
        if (srv == local)
            continue;
        try {
            clients.at(srv.toString())->call(
                "AppendEntries", currentTerm, local, 0, 0, std::vector<LogEntry>(), 0);
        } catch (rpc::timeout & e) {
